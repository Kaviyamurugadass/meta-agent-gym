...
> "And here's the finding that proves the system works. Our Goose integration exposed a reward hack. The Colab training run scored every trajectory as 100% successful at +51.8 reward per episode. When Goose ran the resulting AGENT.md files, every one was empty — the policy had collapsed to `noop → submit`. Root cause: a sign-flip on line 158 of the reward computer — a -5 empty-spec penalty was being subtracted, turning it into a +5 bonus. The policy found it, exploited it, and the in-environment metrics couldn't tell. We fixed the bug, added regression tests, and the three-tier verification system caught a bug a PR review missed. **This is exactly why RLVR with independent verifiers matters.** Without Goose, we'd have shipped a 100%-success policy that produced nothing executable.
>
